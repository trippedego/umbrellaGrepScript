...
parser.add_argument("dnsLog", type=str, nargs="?", help="Windows DNS Log to parse")
...
parser.add_argument('-s', '--smb', nargs=3, help='OPTIONAL: Grab Windows DNS Log via SMB. Use: --smb "Server Hostname" "Username" "Password"')
...
def getFiles(hostname, username, password):
    ...
    assert conn.connect(hostname, 139)

    with open(hostname+'.log', 'wb') as fp:
        conn.retrieveFile('C$', '/DNSLogs/'+hostname+'.txt', fp)
...
def parseWindowsLog(badDomains, timestamp):
    ...
    if args.smb:
        ...
        try:
            getFiles(args.smb[0], args.smb[1], args.smb[2])
