...
def parseLog(badDomains):
    '''
    parseLog(badDomains: list) -> void
    Parse through the DNS log for instances of bad domains, printing to stdout and writing to file,
    badDomains.txt, both the source IP and domain visited with a timestamp.
    '''
    # Open the DNS log, store each line in a list, removing new line and return.
    log = open(args.dnsLog, "r").read().split("\r\n")

    # Open a new file for writing, badDomains.txt in user's current working directory.
    fileBadDomains = open("badDomains.txt", "w")
